...
def annual_values_from_series(raw_series: list[dict], years: int) -> dict[str, float]:
    """
    ...
    That field is reliable for calendar-year filers (confirmed against JNJ
    and P&G), but testing against Apple -- whose fiscal year ends in late
    September, not December -- showed it can be mislabeled for non-calendar
    fiscal years: real, correct revenue figures came back tagged two fiscal
    years newer than they actually were. A reported PERIOD END DATE doesn't
    have that ambiguity no matter what fiscal calendar a company uses, so we
    sort and select on that instead, and label each column with the actual
    end date rather than a bare year number that could be misleading for a
    company like Apple, Nike, or Microsoft.
    """
    ...
    # Keep only the `years` most recent periods, ordered by their real end date
    sorted_ends = sorted(by_period_end.keys())[-years:]
    return {end: by_period_end[end]["val"] for end in sorted_ends}
    ...
    for line_item, tag_candidates in LINE_ITEMS.items():
        ...
        for candidate in tag_candidates:
            if isinstance(candidate, str):
                raw_series = fetch_concept_series(cik, candidate)
                if raw_series:
                    rows[line_item] = annual_values_from_series(raw_series, years)
                    print(f"  -> using tag '{candidate}' ({len(rows[line_item])} years found)")
                    found = True
                    break
                continue

            # Composite candidate: fetch every sub-tag and sum them together,
            # period by period. Only use this candidate if ALL of its sub-tags
            # have data -- a partial sum (e.g. marketing but not G&A) would
            # understate the line item, which is worse than just falling
            # through to the next candidate or the "no data" error.
            sub_series: dict[str, dict[str, float]] = {}
            all_found = True
            for sub_tag in candidate:
                raw_sub = fetch_concept_series(cik, sub_tag)
                if not raw_sub:
                    all_found = False
                    break
                sub_series[sub_tag] = annual_values_from_series(raw_sub, years)

            if all_found and sub_series:
                common_ends = set.intersection(*(set(v.keys()) for v in sub_series.values()))
                combined = {
                    end: sum(sub_series[sub_tag][end] for sub_tag in candidate)
                    for end in common_ends
                }
                if combined:
                    rows[line_item] = combined
                    tag_names = " + ".join(candidate)
                    print(f"  -> using composite '{tag_names}' ({len(combined)} years found)")
                    found = True
                    break

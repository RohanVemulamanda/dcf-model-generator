...
def compute_historical_metrics(financials: pd.DataFrame) -> HistoricalMetrics:
    ...
    Inventory, Accounts Payable) are optional -- if missing, those
    assumptions default to 0. This is exactly the shape produced by
    sec_financials_fetcher.py's build_financial_table().
    """
    revenue = financials.loc["Revenue"].astype(float)
    growth = revenue.pct_change().dropna()

    gross_profit = revenue - financials.loc["Cost of Goods Sold"].astype(float)
    gross_margin = gross_profit / revenue

    sga_pct = financials.loc["SG&A Expense"].astype(float) / revenue
    rd_pct = (financials.loc["R&D Expense"].astype(float) / revenue
              if "R&D Expense" in financials.index else pd.Series(0.0, index=revenue.index))
    da_pct = financials.loc["Depreciation & Amortization"].astype(float) / revenue
    capex_pct = financials.loc["Capital Expenditures"].astype(float).abs() / revenue

    wc_rows = ["Accounts Receivable", "Inventory", "Accounts Payable"]
    if all(r in financials.index for r in wc_rows):

...
def build_workbook(
    ...
    for line_item in historicals.index:
        ...
        for j, yr in enumerate(hist_years):
            ...
            # A line item SEC has no data for under any known tag (e.g. R&D
            # for a retailer that genuinely doesn't report it) comes through
            # as NaN here. Writing that as a literal blank cell violates the
            # "no blank cells" requirement even though it's not a bug in the
            # DCF math -- the engine already treats a missing optional line
            # item as 0. Write 0 here too, so the Excel output matches what
            # the model actually assumes instead of leaving a gap that reads
            # as an error.
